...
class LoRALinear(nn.Module):
    ...
    def __init__(self, base_layer: nn.Linear, r: int = 16, alpha: int = 16):
        ...
        self.A = nn.Parameter(
            torch.randn((r, d_in), dtype=torch.float32, device=device)
        )
        self.B = nn.Parameter(
            torch.zeros((d_out, r), dtype=torch.float32, device=device)
        )
    ...
    def merge(self) -> None:
        ...
        merged = nn.Linear(
            self.base_layer.in_features, self.base_layer.out_features, bias=has_bias
        )
        with (
            torch.no_grad()
        ):  # must turn off autograd for in-place ops on grad-requiring tensors
