...
def inject_lora(model: nn.Module, target_names: list[str], r=16, alpha=16):
    """
    Replaces model's target layers with LoRALinear wrappers.
    ...
    for parent_name, module_name, module in to_replace:
        to_inject = LoRALinear(base_layer=module, r=r, alpha=alpha)
...
def merge_lora(model: nn.Module):
    """
    Merges lora params with base layer in all lora layers
    for reduced inference time.
    """
