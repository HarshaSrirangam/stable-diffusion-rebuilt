...
class DDIM:
    ...
    def step(
        ...
        x0_pred = (latents - ((1.0 - alpha_bar_t) ** 0.5) * noise_pred) / (alpha_bar_t**0.5) # same as DDPM
        # variance is 0 by default (eta=0)
        sigma_t = self.eta * (((1 - alpha_bar_t_prev) / (1 - alpha_bar_t))**0.5) * ((1 - (alpha_bar_t / alpha_bar_t_prev))**0.5)
